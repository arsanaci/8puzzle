...
int main(int argc, char *argv[])
{
  ...
  int B[3][3] =  {{1,2,3}, {8,0,4}, {7,6,5}}; //hedef dizi
   int i, j;
   int d; // istenilen ve girilen dizi arasýndaki fark
   int steps =0 ;
 ...
 { 
	...
	system("PAUSE"); 
       ...
       steps++;
       printf("\nStep: %d \n", steps);
       alter(A, B);    // dizi hangisine uygunsa deðiþtirir
       display(A);
       system("PAUSE");
       }
       system("PAUSE");	
...
int diffup(int A[][3], int B[][3])
{
     ...
     for(i=0;i<3;i++)   //test için A dizisini temp dizininde kopyalama
       for (j=0;j<3;j++)
        temp[i][j] = A[i][j];

     for(i=1;i<3;i++)   // alaný yukarý yönde deðiþtirme
       for (j=0;j<3;j++)    // diziyi i = 1'den baþlatmak, i = 0 koþulunun kaçýnýlmasýný saðlar
        if(A[i][j]==0)         {
               temp[i-1][j] = 0;         
               temp[i][j] = A[i-1][j];
                        }
...
int diffdown(int A[][3], int B[][3])
{
     ...
     for(i=0;i<2;i++)   // boþluðu aþaðý yönde deðiþtirme
       for (j=0;j<3;j++)    //i = 1 ile biten, i = 0 koþulunun kaçýnýlmasýný saðlar
        if(A[i][j]==0)
         {
               temp[i+1][j] = 0;         
               temp[i][j] = A[i+1][j];
...
int diffleft(int A[][3], int B[][3])
{
     ...
     for(i=0;i<3;i++)   // alaný sol yönünde deðiþtirme
       for (j=1;j<3;j++)    
        if(A[i][j]==0)
         {
               temp[i][j-1] = 0;         
...
int diffright(int A[][3], int B[][3])
{
     ...
     for(i=0;i<3;i++)   // alaný sað yönünde deðiþtirme
       for (j=0;j<2;j++)    
        if(A[i][j]==0)
         {
               temp[i][j+1] = 0;         
